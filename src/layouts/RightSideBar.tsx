  ...
  const handleExpandAndShrink = () => {
    if (sideBarRef.current) {
      if (!isExpand) {
        ...
        sideBarRef.current.style.maxWidth = "400px";
      }
      else {
        setIsExpand(false);
        sideBarRef.current.style.minWidth = "0px";
        sideBarRef.current.style.width = "120px";
      }
    }
  }

  return (
    <div className={"w-[120px] h-full flex flex-col items-center pt-7 pb-5 border-l border-l-gray-200 transition-all duration-300 " + (!isExpand && "px-[10px]")} ref={sideBarRef}>
      ...
      <img
        className={"rounded-full object-cover " + (!isExpand ? "w-10 h-10" : "w-32 h-32")}
        src={avatar}
        alt="Rounded avatar"
      />
      {!isExpand
        ...
        <ul className="absolute top-1/2 -translate-y-[50%] w-fit flex flex-col items-center justify-between h-[40%] text-sm font-medium text-gray-500 dark:text-gray-400">
          {
            rightSideBarConfig.map(item => {
              return <li key={item.value}><Tooltip text={item.value} styles='!right-[60%] !w-fit'>{item.icon}</Tooltip></li>;
            })
          }
        </ul>
        :
        <div className="mt-4 w-full flex flex-1 justify-center flex-col gap-4">
          ...
          <Transition
            ...
            className='flex-1'
          >
            <div className='px-5 text-gray-600 text-base text-left font-semibold tracking-wide'>
              File types
            </div>
            <div className='flex flex-col'>
              {
                fileTypeConfig.map(item => <FileType key={item.value} file={item} />)
              }
            </div>
          </Transition>
          <Transition
          ...
          >
            <div className='flex-1 flex flex-col px-5 w-full'>
              <div className='text-gray-600 text-base text-left font-semibold tracking-wide'>
                All links
              </div>
              <div className='flex-1 overflow-hidden'>
                <div className='h-full overflow-auto'>
                  {
                    emptyArray.map((_, index) => <CustomLinkWithTooltip key={index} link='https://www.google.com/webhp?hl=vi&sa=X&ved=0ahUKEwiBvLzxno74AhX1qlYBHY8jC1UQPAgI' toolTipUp={emptyArray.length - 1 - index < 3} />)
                  }
                </div>
              </div>
            </div>
...
const ShareTypeBox = ({ active, switchType, shareType, total }: { active: boolean, switchType: any, shareType: number, total: number }) => {
  return (
    <div
      className={'flex-1 py-7 flex text-center justify-center items-center rounded-xl gap-2 cursor-pointer ' + (active ? 'bg-light-green' : 'bg-neutral-50')}
      onClick={switchType}
    >
      ...
      {
        active && <div className='px-1 py-1 absolute top-2 right-2 bg-white rounded-full'>
    ...
    <div className='w-full flex items-center text-gray-500 hover:text-gray-700 my-2 gap-1'>
      ...
      <div className='w-[90%]'>
        <Tooltip text={link} styles={'w-full !left-0 ' + toolTipUp ? '!bottom-[50%]' : '!top-[100%]'}>
          <a href={link} target="_blank" rel="noopener noreferrer" className='max-w-full text-left block overflow-hidden hover:underline whitespace-nowrap text-ellipsis cursor-pointer'>{link}</a>
        </Tooltip>
      </div>

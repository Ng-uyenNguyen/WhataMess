...
function LoginPage() {
  ...
  const formik = useFormik({
    ...
    validateOnBlur: true,
    validationSchema: Yup.object().shape(pageMode !== 1 ? {
    ...
    <div className="bg-light-green w-full h-full flex justify-center flex-col relative">
      <div className="bg-white xl:w-1/4 lg:w-1/3 md:w-1/2 sm:w-full px-3 py-5 self-center text-center rounded-md shadow-lg">
        ...
        <div className="text-2xl font-semibold text-slate-800">
          {pageMode === 1 ? "Welcome Back" : "Sign Up Your Account"}
        </div>
        <div className="text-gray-400 text-base my-2">
          {pageMode === 1
            ? "Enter your credentials to access your account"
            : "Fill up the information for registering"}
        </div>
        <form className="mx-auto p-4" onSubmit={formik.handleSubmit}>
          ...
          <button
            type="submit"
            className="text-white bg-green hover:bg-green-hover-lighter focus:ring-4 w-full font-medium rounded-lg text-sm px-5 py-2.5 mr-2 uppercase"
          >
      ...
      <Toast type={ToastType.SUCCESS} text="Register successfully!" show={showToast} />
    </div>
  );
}
